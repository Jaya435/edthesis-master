...
The batch size determines the number of training samples that are utilised in one iteration (\cite{mnih13}). Stochastic Gradient Descents (SGDs) are employed in small batch regimes where batch size is typically less than the number of training samples. SGDs are limited for parallelisation due to the sequential natures of iterations, speed-ups are limited by the small batch size (\cite{simonyan14,,mnih13}). However, researchers have observed that this can lead to a loss in generalisation performance (\cite{Keskar16}), which is where the network is intended to perform best. 
\par
